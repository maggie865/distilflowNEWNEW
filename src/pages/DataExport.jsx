...
export default function DataExport() {
  ...
  const toggleAll = (val) => {
    setSelected(Object.fromEntries(ENTITY_NAMES.map(n => [n, val])));
  };
  ...
  const handleExport = () => {
    ...
    if (chosen.length === 0) {
      toast.warning('Select at least one entity to export.');
      return;
    }
    setExporting(true);
    try {
  ...
  return (
    <div className="pb-20 md:pb-0">
      <PageHeader
        title="Data Export"
        subtitle="Monthly backup — export every record in the app to a single downloadable JSON file"
      >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Card><CardContent className="p-4">
          ...
          <p className="text-2xl font-bold font-display mt-1">{rows.length}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
          ...
          <p className="text-2xl font-bold font-display mt-1">{totalEntitiesReady}/{rows.length}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
          ...
          <p className="text-2xl font-bold font-display mt-1">{totalRecords.toLocaleString()}</p>
        </CardContent></Card>
        <Card><CardContent className="p-4">
          ...
          <p className="text-2xl font-bold font-display mt-1">{selectedRecords.toLocaleString()}</p>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                id="select-all"
              />
              <label htmlFor="select-all" className="text-sm font-medium cursor-pointer">Select all entities</label>
            </div>
            <p className="text-xs text-muted-foreground">
              Backup includes all fields for each record. Run this monthly to keep an offline copy of your data.
            </p>
          </div>
