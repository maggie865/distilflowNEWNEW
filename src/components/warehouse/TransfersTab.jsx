...
export default function TransfersTab({ warehouseStock, onPrintSlip }) {
  ...
  const totalBottles = sorted.reduce((s, w) => s + (w.quantity_bottles || 0), 0);
  const totalLALs = sorted.reduce((s, w) => s + (w.total_lals || 0), 0);

  const dateSubtotals = useMemo(() => {
    const map = {};
    sorted.forEach(w => {
      const date = w.transfer_date || '—';
      if (!map[date]) map[date] = { bottles: 0, lals: 0 };
      map[date].bottles += w.quantity_bottles || 0;
      map[date].lals += w.total_lals || 0;
    });
    return map;
  }, [sorted]);
      ...
      <div className="rounded-lg border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {renderRows.length === 0 ? (
                <TableRow><TableCell colSpan={9} className="text-center py-8 text-muted-foreground">No transfers in this period</TableCell></TableRow>
              ) : renderRows.map(row => {
                if (row.type === 'header') {
                  const sub = dateSubtotals[row.date] || { bottles: 0, lals: 0 };
                  return (
                ...
                return (
                  <TableRow key={row.key}>
                    ...
                    <TableCell className="text-right text-sm">{(w.quantity_bottles || 0).toLocaleString()}</TableCell>
